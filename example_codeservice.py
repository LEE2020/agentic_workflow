...
from eval_component import print_html
...
db = inv_utils.db
inventory_tbl = inv_utils.inventory_table
transactions_tbl = inv_utils.transactions_table
...
def customer_service_agent(
    ...
    *,
    ...
    print_html(full_content, title="Plan with Code (Full Response)")

    # 3) Before snapshots
    print_html(json.dumps(inventory_tbl.all(), indent=2), title="Inventory Table · Before")
    print_html(json.dumps(transactions_tbl.all(), indent=2), title="Transactions Table · Before")
    ...
    print_html(exec_res["answer"], title="Plan Execution · Extracted Answer")
    print_html(json.dumps(inventory_tbl.all(), indent=2), title="Inventory Table · After")
    print_html(json.dumps(transactions_tbl.all(), indent=2), title="Transactions Table · After")
